...
Location: `/home/raymond/workspace/projets/skills_and_mcp/RSVS`
...
## 2) Start Frontend
```bash
cd /home/raymond/workspace/projets/skills_and_mcp/RSVS/frontend
npm install
npm run dev
```
...
- `NEXT_PUBLIC_RSVS_BACKEND_URL` (default `http://127.0.0.1:8787`)
...
4. Backend writes artifacts to:
   - `RSVS/atom/snapshot-*.json`
   - `RSVS/atom/events-*.jsonl`
   - `RSVS/atom/report-*.json`
