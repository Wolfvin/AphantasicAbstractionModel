...
│  │                                                               │ │
│  │  ┌──────────┐  ┌──────────┐  ┌──────────┐  ┌────────────┐  │ │
│  │  │ 3D Force │  │ Appraise │  │  Relate  │  │  Timeline  │  │ │
│  │  │  Graph   │  │  Panel   │  │  Panel   │  │  + HUD     │  │ │
│  │  └──────────┘  └──────────┘  └──────────┘  └────────────┘  │ │
│  │                                                               │ │
│  │  Zustand Store ◄──── backendBridge.ts ────► HTTP API        │ │
│  └───────────────────────────┬─────────────────────────────────┘ │
└──────────────────────────────┼───────────────────────────────────┘
                               │
                    HTTP (port 8000)
...
│                              │                                     │
│  ┌────────────────┐  ┌───────▼────────┐  ┌────────────────────┐ │
│  │ artifacts.py   │  │ conversion.py  │  │   config.py        │ │
│  │ (file I/O)     │  │ (format xform) │  │ (constants, paths) │ │
│  └────────────────┘  └────────────────┘  └────────────────────┘ │
│                              │                                     │
│                    ┌─────────▼──────────┐                         │
│                    │   rsvs_core.py     │                         │
...
│  │   ingest_text() · appraise() · relate() · snapshot_v1()  │    │
│  └──────┬─────────┬──────────┬──────────┬──────────┬────────┘    │
│         │         │          │          │          │              │
│  ┌──────▼──┐ ┌────▼────┐ ┌──▼───┐ ┌───▼────┐ ┌──▼──────┐      │
│  │graph.rs │ │attention│ │sense │ │autonomy│ │persist  │      │
│  │(DAG +   │ │  .rs    │ │ .rs  │ │  .rs   │ │  .rs    │      │
│  │ Jaccard)│ │(NPMI +  │ │(multi│ │(EMA +  │ │(JSON    │      │
│  │         │ │ Jaccard │ │sense │ │hysteres│ │ snapshot│      │
│  │         │ │ + cooc) │ │clust)│ │is)     │ │)        │      │
│  └────┬────┘ └─────────┘ └──────┘ └────────┘ └─────────┘      │
│       │                                                          │
│  ┌────▼────┐  ┌──────────┐  ┌──────────┐  ┌────────────────┐   │
│  │seed.rs  │  │events.rs │  │types.rs  │  │  bindings.rs   │   │
│  │(24 atoms│  │(stream)  │  │(v4.2     │  │(PyO3 #[cfg(   │   │
│  │ axioms) │  │          │  │ models)  │  │feature=python])│   │
│  └─────────┘  └──────────┘  └──────────┘  └────────────────┘   │
└──────────────────────────────────────────────────────────────────┘
```
...
### Ingest Data Flow (ASCII)

```
POST /run {mode: "ingest", text: "Water flows..."}
         │
         ▼
   ┌─────────────────────────────────────────────────────┐
   │  Python Bridge (modes.py → rsvs_core.py)             │
   │                                                       │
   │  1. _normalize_view(options.view)                     │
   │  2. _normalize_lang(options.language)                 │
   │  3. r.ingest_with_meta_v1(text, domain_id)           │
   └──────────────────────┬──────────────────────────────┘
                          │ PyO3
   ┌──────────────────────▼──────────────────────────────┐
   │  Rust Core (pipeline.rs)                              │
   │                                                       │
   │  1. text_to_sentences(text) → [sentence_tokens]      │
   │  2. For each sentence:                                │
   │     a. stats_db.ingest_sentence(tokens)               │
   │     b. entities.record(token, groundable)             │
   │  3. entities.candidates(N=3) → promotion_list        │
   │  4. For each candidate:                               │
   │     a. graph.insert_node(v4.2 Node)                   │
   │     b. autonomy.register(id, 0.50, Tier2)            │
   │     c. events.emit("node_created")                    │
   │  5. For each sentence (attention loop):               │
   │     a. attention.select(tokens, stats, atom_sets)     │
   │     b. For each token:                                │
   │        - sense.ingest(context) → IngestResult         │
   │        - autonomy.update_confidence(...)              │
   │        - events.emit(...)                             │
   │     c. Every 20 contexts: merge + purge fragile       │
   │  6. autonomy.check_global_stability()                 │
   │     → rollback if delta > threshold                   │
   └──────────────────────┬──────────────────────────────┘
                          │
   ┌──────────────────────▼──────────────────────────────┐
   │  Python Bridge (modes.py → conversion.py)             │
   │                                                       │
   │  1. r.snapshot_v1() → snapshot_raw                   │
   │  2. r.consume_events_v1(seq_before, 10000) → events  │
   │  3. _build_bridge_snapshot(snapshot_raw, ...)         │
   │  4. _convert_rust_event(...) for each event          │
   │  5. _save_rsvs() (persist Rust state to disk)        │
   │  6. _write_ingest_artifacts(snapshot, events, stats) │
   └──────────────────────────────────────────────────────┘
```
...
```
  New context arrives
         ...
         ▼
  ┌──────────────────┐     No existing senses     ┌─────────────────┐
  │  Increment       │──────────────────────────→ │ Create Fragile  │
  │  inactivity      │                             │ Sense (N=1)     │
  │  for all senses  │                             └─────────────────┘
  └────────┬─────────┘
           │ Existing senses exist
           ▼
  ┌──────────────────┐     No qualifying sense    ┌─────────────────┐
  │  Score each       │     score ≥ θ_assign      │ Create Fragile  │
  │  existing sense   │                            │ Sense (N=1)     │
  │  (candidate       │                            └─────────────────┘
  │   pruning)        │
  └────────┬─────────┘
           │ Best score ≥ θ_assign
           ▼
  ┌──────────────────┐     N < 2                  ┌─────────────────┐
  │  Assign context  │──────────────────────────→ │  Stay Fragile   │
  │  to best sense   │                             │  (inactivity=0) │
  └────────┬─────────┘                             └─────────────────┘
           │ N ≥ 2
           ▼
  ┌──────────────────┐     Jaccard ≥ θ_merge     ┌─────────────────┐
  │  Mature Sense    │──────────────────────────→ │  Merge senses   │
  │  (N≥2, stable)   │                            │  (pool coherence│
  └──────────────────┘                            │   + frequencies)│
                                                  └─────────────────┘
  Periodic maintenance:
  ┌──────────────────┐     inactivity ≥ k_fragile
  │  Fragile senses  │──────────────────────────→ PRUNED (deleted)
  │  (N=1 only)      │
  └──────────────────┘
```
...
| `verified_runtime` | 0.80 |
| `user_input` | 0.65 |
| `unknown_external` | 0.40 |
...
The gap between promotion (≥ 0.75) and demotion (< 0.60) thresholds prevents rapid oscillation. When it does occur (3+ flips), the node is **quarantined** — a circuit-breaker pattern.
...
|------|--------|-------------|
| `snapshot-{timestamp}.json` | JSON | Full knowledge graph snapshot |
| `events-{timestamp}.jsonl` | JSONL | Event stream (one JSON per line) |
| `report-{timestamp}.json` | JSON | Ingest statistics report |
| `appraise-{timestamp}.json` | JSON | Appraise mode result |
| `relate-{timestamp}.json` | JSON | Relate mode result |
| `rsvs-state.json` | JSON | Full Rust core state (for warm restart) |
...
| `ForceGraph.tsx` | Force-directed layout with tier-weighted repulsion |
| `GraphNode.tsx` | Individual node rendering (sphere with tier/color) |
| `GraphEdge.tsx` | Edge rendering (lines with weight-based thickness) |
| `LeftInputRail.tsx` | Text input + mode selection |
| `RightNodeDrawer.tsx` | Node detail panel with v4.2 metadata |
| `AppraisePanel.tsx` | Appraise mode results (verdict, stance, evidence) |
| `RelatePanel.tsx` | Relate mode results (related nodes/edges) |
| `TimelineBar.tsx` | Event timeline with play/pause/speed controls |
| `GraphHUD.tsx` | Heads-up display overlay (stats, mode) |
...
### Frontend Data Flow (ASCII)

```
  ┌─────────────────────────────────────────────────────────────┐
  │  User Interaction                                            │
  │  ┌──────────────┐  ┌───────────────┐  ┌────────────────┐   │
  │  │ Text Input    │  │ Node Click    │  │ Timeline Play  │   │
  │  │ + Mode Select │  │ (select node) │  │ (replay events)│   │
  │  └──────┬───────┘  └──────┬────────┘  └───────┬────────┘   │
  └─────────┼─────────────────┼───────────────────┼────────────┘
            │                 │                   │
            ▼                 ▼                   ▼
  ...
  │  Zustand Stores                                              │
  │  ┌──────────────┐  ┌───────────────┐  ┌────────────────┐   │
  │  │ useRSVSStore │  │ useModeResult │  │ Timeline Store │   │
  │  │ (graph data) │  │ Store (appr/  │  │ (events)       │   │
  │  │              │  │ relate result)│  │                │   │
  │  └──────┬───────┘  └──────┬────────┘  └───────┬────────┘   │
  └─────────┼─────────────────┼───────────────────┼────────────┘
            │                 │                   │
  ...
  │  Rendering Layer                                             │
  │  ┌──────────────┐  ┌───────────────┐  ┌────────────────┐   │
  │  │ ForceGraph   │  │ RightNode     │  │ TimelineBar    │   │
  │  │ (3D scene)   │  │ Drawer        │  │ + Appraise/    │   │
  │  │              │  │ + Panels      │  │ Relate Panels  │   │
  │  └──────────────┘  └───────────────┘  └────────────────┘   │
  └─────────────────────────────────────────────────────────────┘
```
...
All communication goes through `backendBridge.ts` which wraps `fetch()` calls.

### Node Rendering

The frontend computes all visual properties independently from the bridge:

| Property | Computed From | Logic |
|----------|--------------|-------|
| Size | `tier`, `is_seed` | Seed=1.6, T1=1.4x, T2=1.0x, T3=0.7x |
| Color | `status` | stable=green, candidate=cyan, new=purple, deprecated=orange, quarantine=red |
| Glow | `confidence` | 0.3–0.9 range |
| Opacity | `confidence` | 0.4–1.0 range |
| Edge thickness | `weight` | Linear mapping |
...
> 📖 For complete API documentation with all endpoints, request/response schemas, and error codes, see [docs/API.md](API.md)
...
### Why PyO3 (Not C FFI or HTTP IPC)?

| Approach | Safety | Performance | Ergonomics |
|----------|--------|-------------|------------|
| C FFI | ❌ Manual memory management | ✅ Direct | ❌ Complex error handling |
| HTTP IPC | ✅ Process isolation | ❌ Serialization overhead | ✅ Simple |
| PyO3 | ✅ Safe Rust types | ✅ Near-zero overhead | ✅ Native Python types |

PyO3 provides the best combination of safety, performance, and developer experience for Rust↔Python interop.
