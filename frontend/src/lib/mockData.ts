...
function formatEventLabel(evt: RSVSEvent): string {
  switch (evt.event_type) {
    case 'atom_created':
      return `Atom "${(evt.payload.node as any)?.label || '?'}" created`;
    case 'edge_created':
      return `Edge ${(evt.payload.edge as any)?.id || '?'} connected`;
    case 'confidence_changed':
      return `Confidence updated for ${(evt.payload.node as any)?.label || '?'}`;
    case 'edge_weight_changed':
      return `Weight changed for edge ${(evt.payload.edge as any)?.id || '?'}`;
    case 'tier_changed':
      return `Tier changed for ${(evt.payload.node as any)?.label || '?'}`;
    case 'atom_removed':
      return `Atom ${(evt.payload.node as any)?.label || '?'} removed`;
    default:
