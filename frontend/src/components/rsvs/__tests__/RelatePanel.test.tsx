...
function makeRelateResult(overrides: Partial<RelateResult> = {}): RelateResult {
  return {
    ...
    related_nodes: [
      { node_id: 1, label: 'mineral', score: 0.9, tier: 1, kind: 'node' },
      { node_id: 2, label: 'sediment', score: 0.6, tier: 2, kind: 'node' },
    ],
    related_edges: [
...
describe('RelatePanel', () => {
  ...
  it('shows node kind badges', () => {
    const result = makeRelateResult();
    render(<RelatePanel result={result} />);
    // Both nodes are 'node' kind
    const nodeBadges = screen.getAllByText('Node');
    expect(nodeBadges.length).toBeGreaterThanOrEqual(1);
  });
  ...
  it('does not show related edges section when empty', () => {
    const result = makeRelateResult({
      related_nodes: [{ node_id: 1, label: 'test', score: 0.5, tier: 1, kind: 'node' }],
      related_edges: [],
    });
    render(<RelatePanel result={result} />);
