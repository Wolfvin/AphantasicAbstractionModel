...
pub struct PipelineConfig {
    ...
    /// N≥ this to promote CANDIDATE_ID to atom
    pub entity_promote_n: usize,

    /// Seed atom labels (for grounding check)
    pub seed_labels: Vec<String>,

    /// Domain tag for current ingestion batch (used by autonomy engine)
    /// Increment this when switching to a new domain/source
...
pub struct QueryResult {
    ...
    pub scored_atoms:     Vec<(String, f32)>, // (label, score) sorted desc
}

// -----------------------------------------------------------------------
// Rsvs — the main system struct
// -----------------------------------------------------------------------

pub struct Rsvs {
    pub graph:    RsvsGraph,
    pub senses:   HashMap<NodeId, SenseManager>,  // per-ID sense manager
...
impl Rsvs {
    /// Create a new RSVS instance and bootstrap seed atoms.
    pub fn new(config: PipelineConfig) -> Self {
        ...
        for (label, &id) in &seed_map {
            autonomy.register(id, 1.0, Tier::Tier1);
    ...
    pub fn snapshot_v1(&self) -> RuntimeSnapshot {
        let nodes = self.graph.nodes.values().map(|n| {
            ...
            RuntimeNode {
                id: n.id,
                label: n.label.clone().unwrap_or_else(|| format!("#{}", n.id)),
                kind: if n.kind == NodeKind::Atom { "atom".into() } else { "composite".into() },
                tier: match n.tier {
                    Tier::Tier1 => 1,
                    Tier::Tier2 => 2,
                    Tier::Tier3 => 3,
    ...
    // -----------------------------------------------------------------------
    // Main entry: ingest a block of text
    // -----------------------------------------------------------------------
    ...
    pub fn ingest_text(&mut self, text: &str) -> IngestStats {
        ...
        // --- Step 2: Promote new entity candidates to atoms ---
        ...
        // --- Step 2b: Build/update composite definitions for promoted entities ---
        // A composite's atom set = top co-occurring atoms from attention
        // This enables sim(stone, bone) to find shared atoms like "hard", "solid"
        self.update_composites(&candidates, &correlation_id);
        ...
        for tokens in &sentences {
            ...
            // Run RSVS Attention: select relevant pairs
            ...
            // For each token that has a known ID, build its context atom set
            // from its selected candidates
            for token in tokens {
                ...
                // Feed context into this token's SenseManager
                ...
                // Get coherence of the active sense for confidence update
                let active_coherence = sense_mgr.senses.first()
                    .map(|s| s.coherence)
                    .unwrap_or(0.5);

                // Frequency: 1.0 if token appears (it always does here)
                let freq = 1.0f32;

                // Update atom confidence
            ...
            // Periodic sense maintenance: merge + purge fragile
        ...
        // --- Step 4: Check global stability, rollback if needed ---
    ...
    // -----------------------------------------------------------------------
    // Build/update composite definitions from co-occurrence data
    // -----------------------------------------------------------------------

    fn update_composites(&mut self, tokens: &[String], correlation_id: &str) {
        use crate::types::NodeKind;
        ...
        for token in tokens {
            ...
            // Skip if already a composite with atoms
            if let Some(node) = self.graph.get_node(token_id) {
                if node.kind == NodeKind::Composite && !node.atoms.is_empty() {
                    continue; // already built
                }
            }

            // Collect top co-occurring atoms from stats (those with known IDs)
            let mut cooc_atoms: Vec<(NodeId, f32)> = self.token_to_id
                .iter()
            ...
            if cooc_atoms.is_empty() { continue; }

            // Sort by co-occurrence strength, take top 8
            cooc_atoms.sort_by(|a, b| b.1.partial_cmp(&a.1).unwrap());
            cooc_atoms.truncate(8);

            let atom_ids: Vec<NodeId> = cooc_atoms.iter().map(|(id, _)| *id).collect();

            // Update node to composite with these atoms
            // We do this by re-inserting — but since DAG doesn't support mutation,
            // we store atom membership in the sense manager's freq_map instead.
            // For similarity, override expand() by storing atoms in the node:
            if let Some(node) = self.graph.nodes.get_mut(&token_id) {
                ...
                for (source_id, weight) in &cooc_atoms {
    ...
    /// Given a concept label and a query context string,
    /// return the most relevant atoms from the active sense.
    pub fn query(&self, concept: &str, query_context: &str) -> Option<QueryResult> {
        ...
        // Context lookup: P(a|S,q) ∝ freq_S(a) × P(q|a)
        // For atoms in active sense's core, score by freq
